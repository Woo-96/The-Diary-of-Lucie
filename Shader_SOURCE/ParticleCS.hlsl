            ...
            while (0 < SpawnCount)
            {
                ...
                if (orgvalue == outvalue)
                {
                    ...
                    if (ModuleData.AddVelocity)
                    {
                        ...
                        {
                            float3 Dir = normalize(particle.vLocalPos.xyz);
                            particle.vVelocity.xyz = -1.f * Dir * ModuleData.Speed;
                        }
                        
                        // Fixed Direction
                        else
                        {
                            float3 Dir = ModuleData.vVelocityDir.xyz;
                            if (0 != ModuleData.OffsetAngle)
                                Dir.xy = float2(cos(radians(ModuleData.OffsetAngle)), -sin(radians(ModuleData.OffsetAngle)));
                            particle.vVelocity.xyz = Dir * ModuleData.Speed;
                        }
                    }
