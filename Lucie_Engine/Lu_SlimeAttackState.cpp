...
namespace Lu
{
	...
	void SlimeAttackState::Exit()
	{
		...
		// 투사체 생성 위치 조정
		Vector3 vMonPos = GetTransform()->GetPosition();
		vMonPos.y -= 35.f;

		// 투사체 생성과 설정
		for (int i = 0; i < 3; ++i)
		{
			GameObject* pProjectile = object::Instantiate<GameObject>(vMonPos, Vector3(70.f, 70.f, 100.f), eLayerType::MonsterProjectile);
